        ...
        <div className="flex flex-col gap-1">
                    ...
                    {selectedIds.map((id, idx) => {
                        ...
                        return (
                            <div
                                ...
                                className={`flex items-center gap-2 px-1 py-1 rounded cursor-grab hover:bg-blue-50 ${dragIdx === idx ? "opacity-50" : ""}`}
                            >
                                ...
                                <span className="ml-auto text-[10px] text-blue-500 font-semibold">#{idx + 1}</span>
                            </div>
                        );
                    })}
            ...
            {unselectedCats.map((cat) => (
                <label key={cat._id} className="flex items-center gap-2 px-1 py-1 rounded cursor-pointer hover:bg-gray-50">
                    ...
                    <input type="checkbox" checked={false} onChange={() => toggle(cat._id)} className="w-3.5 h-3.5 accent-blue-600" />
                    <span className="text-xs text-gray-700">{cat.title}</span>
                </label>
            ))}
        </div>
